...
public class Tree extends LandscapeObject{
	 ...
	 * @return	void. 
	 */
	public void rotate(double radians, int i, int j) {
		AffineTransform at = new AffineTransform();
        at.rotate(radians, i, j);
        g2.transform(at);
	...
	private void drawBranches() {
		int x = super.getStartX();
		int y = super.getStartY();

		for (int i=0; i <levels; i++) { 
			Double branchA = branchHeight * branchOverlap * i;
			int[] xPoints = {x - (branchWidth/2), x + (branchWidth/2), x};
			int[] yPoints = {y - branchA.intValue(), y - branchA.intValue(), y - branchHeight - branchA.intValue()};
			var branch = new Polygon(xPoints, yPoints, 3);
			g2.setColor(Color.decode(branchColor));
			g2.fill(branch);
			branchWidth = (int)(branchWidth* 0.9);
		}
